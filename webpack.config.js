...
function createConfig(mode, entry, output, plugins) {
    return {
        ...
        resolve: {extensions: ['.tsx', '.ts', '.jsx', '.js', '.json']}, externals: {
            _require: 'require',
            photoshop: 'commonjs2 photoshop',
            uxp: 'commonjs2 uxp',
            os: 'commonjs2 os',
        }, output: {
            filename: '[name].js', path: output,
        },
...
module.exports = (env, argv) => {
    ...
    const uxpPanelConfig = createConfig(argv.mode, {uxp: './src/index.tsx'},
        path.join(dist, panelName), [
            new webpack.ProvidePlugin({
                _require: '_require',
            }), new HtmlWebpackPlugin({
                template: './src/index.html', filename: 'index.html', /*inlineSource: '.(js)$',*/
                chunks: ['uxp'],
